...
async def set_text_command(
    ...
    try:
        ...
        await update.message.reply_text(
            f"❌ {str(e)}\n\nUsage: `/set_text on` or `/set_text off`", parse_mode="Markdown"
        )
        return
    ...
    if not settings:
        await update.message.reply_text(
            "❌ Please run `/setup` first to initialize group settings.", parse_mode="Markdown"
        )
        return

    status = "✅ Enabled" if store_text else "❌ Disabled"
    await update.message.reply_text(f"📝 Text storage has been **{status}**", parse_mode="Markdown")
...
async def set_reactions_command(
    ...
    await update.message.reply_text(
        f"🎭 Reaction capture has been **{status}**", parse_mode="Markdown"
    )

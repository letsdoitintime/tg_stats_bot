...
class PluginManager:
    ...
    def _load_config(self) -> dict:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                self._logger.error(
                    "failed_to_load_plugin_config", file=self.config_file, error=str(e)
                )
    ...
    def _should_watch_file(self, filepath: Path) -> bool:
        ...
        ignore_patterns = settings.get(
            "ignore_patterns", ["README.md", "*.md", "__pycache__", "*.pyc", ".git"]
        )
        ...
        for plugin_dir in self.plugin_dirs:
            ...
            for item in plugin_path.iterdir():
                ...
                try:
                    ...
                    self._logger.error(
                        "failed_to_load_plugin_module", item=str(item), error=str(e), exc_info=True
                    )
        ...
        for plugin_class in plugin_classes:
            try:
                ...
                if not self._is_plugin_enabled_in_config(plugin_name):
                    self._logger.info("plugin_disabled_in_config", plugin=plugin_name)
                    continue
                ...
                plugin_config = (
                    self.plugin_config.get("plugins", {}).get(plugin_name, {}).get("config", {})
                )
                ...
                try:
                    ...
                    package_config_file = os.path.join(
                        self.plugin_dirs[0], module_name, "plugin.yaml"
                    )
    ...
    async def initialize_plugins(self, app: Application) -> None:
        ...
        for plugin_name, plugin in self._plugins.items():
            try:
                ...
                self._logger.error(
                    "plugin_initialization_failed", plugin=plugin_name, error=str(e), exc_info=True
                )
                plugin.disable()
    ...
    async def shutdown_plugins(self) -> None:
        ...
        for plugin_name, plugin in self._plugins.items():
            try:
                ...
                self._logger.error(
                    "plugin_shutdown_failed", plugin=plugin_name, error=str(e), exc_info=True
                )
        ...
        for plugin_name, plugin in self._command_plugins.items():
            ...
            try:
                ...
                for command_name, handler in commands.items():
                    ...
                    self._logger.info(
                        "command_registered", plugin=plugin_name, command=command_name
                    )
            except Exception as e:
                self._logger.error(
                    "failed_to_register_commands", plugin=plugin_name, error=str(e), exc_info=True
                )
            ...
            try:
                ...
                for file_path, old_mtime in list(self._file_mtimes.items()):
                    try:
                        ...
                        self._logger.error(
                            "error_checking_file_mtime", file=file_path, error=str(e)
                        )
                ...
                self._logger.error("hot_reload_error", error=str(e), exc_info=True)
    ...
    async def _reload_plugins(self, app: Application) -> None:
        ...
        for plugin_name, plugin in list(self._plugins.items()):
            try:
                ...
                self._logger.error(
                    "error_shutting_down_plugin_for_reload", plugin=plugin_name, error=str(e)
                )
        ...
        self._logger.info(
            "plugins_reloaded", count=len(self._plugins), plugins=list(self._plugins.keys())
        )

...
def _parse_handler_args(
    args: tuple,
) -> Tuple[Optional[Any], Update, ContextTypes.DEFAULT_TYPE, tuple]:
    """
        ...
        async with async_session() as session:
            try:
                result = await _call_handler(
                    func, self_arg, update, context, extra_args + (session,), kwargs
                )
...
def log_handler_call(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            logger.error("Handler failed", handler=func.__name__, error=str(e), exc_info=True)
            raise
...
def require_admin(func: Callable) -> Callable:
    ...
    async def wrapper(*args, **kwargs) -> Any:
        ...
        try:
            ...
            if update.effective_message:
                await update.effective_message.reply_text("❌ Error checking permissions.")

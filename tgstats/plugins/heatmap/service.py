...
class HeatmapService:
    ...
    async def get_hourly_activity(
        self, chat_id: int, days: int = 7, use_cache: bool = True
        ...
        if use_cache:
            ...
            if cached_data:
                logger.info("heatmap_cache_hit", chat_id=chat_id, days=days)
        ...
        logger.info(
            "heatmap_query_started", chat_id=chat_id, days=days, message_count=message_count
        )
        ...
        if use_cache:
            cache_key = self._get_cache_key(chat_id, days)
            await cache_manager.set(cache_key, json.dumps(serializable_data), ttl=self.CACHE_TTL)
            logger.info(
                "heatmap_cached", chat_id=chat_id, days=days, data_points=len(serializable_data)
            )
